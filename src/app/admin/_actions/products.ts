...
export async function updateProduct(id: string, prevState: unknown, formData: FormData) {
  ...
  if (data.file != null && data.file.size > 0){
    ...
    const rawFilePath = `products/${crypto.randomUUID()}-${data.file.name}`
    filePath = rawFilePath.replace(/\s/g, '_')
  ...
  if (data.image != null && data.image.size > 0){
    ...
    const rawImagePath = `products/${crypto.randomUUID()}-${data.image.name}`
    imagePath = rawImagePath.replace(/\s/g, '_')
